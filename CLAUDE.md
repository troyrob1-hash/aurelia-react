...
- **P&L opening-value reconciliation bug (found 2026-06-18) — NEEDS FRESH INVESTIGATION, do not fix tired.** Best Buy W2 (`2026-P06-W2`) surfaced three "opening" values that should all be the same number but aren't: UI displayed **$2.64**, stored `openingValue` on the W2 P&L doc = **$18.54** (last `updatedAt` 2026-06-16, predates this week's W1 counts), and W1's stored `closingValue` (the supposed source of W2 opening) = **$0**. None of the three reconcile. The opening pipeline is matching neither the stored value nor a live recompute of the prior week's closing. Three related findings on the same location: (1) **W1 has a stale Approved Sysco invoice** — `glCode=50413`, $1,167.40, `invoiceNum=647615036`, approved before commit `550a6c9` deployed, so W1's `cogs_purchases` field is absent ($0 effective) when it should be $1,167.40. Re-approve (or markPaid) that one invoice to trigger the recompute path and backfill. (2) **CR_ prefix path confusion** — invoice `location` field is literally `"CR_Best Buy"` (legacy prefix), `cleanLocName` strips it for display, but Firestore paths sanitize the raw name → `CR_Best_Buy`. Earlier reads at the `Best_Buy` path returned "doc not found" because the prefix was missing. Any audit/diagnostic script must use the raw prefixed location for `locId()`, not the display name. (3) W2 `cogs_inventory` is currently $0 — clamped from a negative by `Math.max(0, openingValue + purchases − closingValue)` in `usePnL.js` / `pnlRollup.js`. **Don't remove the clamp** until the opening pipeline is sorted out, or every location with stale opening data will show negatives. **Investigation approach when fresh:** trace the full opening→closing→purchases pipeline end-to-end starting from `usePnL.js` and `pnlRollup.js`, identify the canonical source for each value, confirm whether the W2 `openingValue` field is supposed to be authoritative or recomputed on read, and reconcile with the W1 closing path. Diagnostic script `scripts/check-cr-bestbuy.cjs` is the reference for the data layout.
